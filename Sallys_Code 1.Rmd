...
metabolics_uncleaned2 <- read_xlsx("Metabolic Syndrome Data (2).xlsx", sheet = 1)

# Checking if columns names are the order the same column names
colnames(metabolics_uncleaned) == colnames(metabolics_uncleaned2)
```
...
metabolics_uncleaned <- rbind(cleaner(metabolics_uncleaned), cleaner(metabolics_uncleaned2)) %>% distinct()
...
metabolics <- metabolics_uncleaned %>%
  mutate(
    # Country of origin
...
## TO BE DONE BY KAYLEE
#%>% 
#mutate(# Acculturation
#acculturation = case_when(
#(eng_speak_ability %in% c("High")) & (eng_read_ability %in% c"High")) ~ "High English Proficiency",
#(eng_speak_ability %in% ("Moderate", "High")) & (eng_read_ability %in% c("Moderate", "High"))  ~ "Mid English Proficiency",
#(eng_speak_ability %in% c("Moderate")) & (eng_read_ability %in% c("High"))  ~ "Mid English Proficiency",
#(eng_speak_ability %in% c("Moderate")) & (eng_read_ability == "Low")  ~ "Mid English Proficiency",
#(eng_speak_ability == "Poor") & (eng_read_ability == "Poor") ~ "Low English Proficiency",
#TRUE ~ NA_character_))
```
